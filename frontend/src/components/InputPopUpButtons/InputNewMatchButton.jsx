...
export default function InputNewMatchButton({ teamId, players, onMatchAdded }) {
  ...
  function handleRadioChange(event) {
    let homeRadioButton =
      document.querySelector("#inp-home").parentNode.parentNode;
    let awayRadioButton =
      document.querySelector("#inp-away").parentNode.parentNode;
  ...
  function handleSelectChange() {
    ...
    for (let p of players) {
      ...
      if (allSelectValues.indexOf(playerString) === -1)
        //if the player is not one of the already chosen players
  ...
  async function handleSubmit(formData, close) {
    ...
    const location = formData.get("location") === "home" ? "HOME" : "AWAY";
    const inPlayers = [
          ...
          <div className={styles.modalHeader}>
            <div className={styles.modalHeaderLeft}>
              ...
              <div>
                ...
                <p className={styles.modalSubtitle}>Udfyld kampens detaljer</p>
              </div>
            </div>
            <button
            ...
            <div className={styles.formBody}>
              ...
              <div className={styles.fieldGroup}>
                <span className={styles.label}>
                  Lokation <span className={styles.required}>*</span>
                </span>
                <div className={styles.twoFields}>
                  <label
                  ...
                  {[1, 2, 3, 4, 5, 6, 7].map((number, index) => {
                    return (
                      <select
                        key={number}
                        className={styles.input}
                        id={`inp-inPlayer${number}`}
                        name={`inPlayer${number}`}
                          ...
                          if (index <= innerIndex)
                            return (
                              <option
                                ...
                                value={CreatePlayerString(player)}
                              ></option>
                            );
                        })}
                      </select>
